...
以下一些操作可以到[learngitbranching](https://learngitbranching.js.org/?NODEMO)上进行交互式演示, 接下来在讲解相关概念的时候会在这里做相关的命令演示, 小伙伴们可以在上面操作一下

注: 
1. 以下的所有commitid都可以换成branch(或者说ref)<!-- 更能理解其实ref就是对某个commitid的引用 -->
...
- 代码稳定后尽快推送(尤其针对公共内容的改动) <!-- 增加上传频率, 小伙伴们更容易获取到最新的代码 -->
...
<!-- 比如多个项目共用一个dal层的项目, 就可以把数据库的repo作为一个公共的子模块给其他的项目使用, 但同时我们也可以在其他的项目中修改dal层的项目代码, 然后提交子模块 -->